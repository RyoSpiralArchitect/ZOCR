...
def main():
    ...
    if argv and argv[0] in {"history", "summary", "plugins", "report", "diagnose", "inspect"}:
        ...
        if cmd == "history":
            ...
            recs = load_history(hargs.outdir)
            print_history(recs, None if hargs.full or hargs.limit <= 0 else hargs.limit)
            return
        if cmd == "summary":
            ...
            try:
                data = read_summary(sargs.outdir)
        ...
        if cmd == "report":
            ...
            path = generate_report(rargs.outdir, dest=rargs.dest, limit=limit)
