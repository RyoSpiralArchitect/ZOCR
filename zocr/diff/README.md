...
- **[JA]** 数値が出てこない文章差分でも `textual_changes` エントリを生成し、`text_change_type` と `line_similarity` を `assist_plan.json` へ引き継ぐため、「備考の追記」「条文差し替え」などを下流エージェントが正しく把握できます。
- **[EN]** Text-only adjustments now emit `textual_changes` entries whose `text_change_type` and `line_similarity` propagate into `assist_plan.json`, letting downstream agents distinguish inserted notes versus rewritten clauses even without numeric motion.
- **[FR]** Les modifications purement textuelles produisent aussi des entrées `textual_changes` dont `text_change_type` et `line_similarity` sont transmis à `assist_plan.json`, ce qui informe clairement les agents aval (nouvelle note, clause réécrite, etc.).
...
- **[JA]** 数値を含まない備考や文章の差し替えでも `textual_changes` が出力され、`text_change_type`（added / removed / modified）と `line_similarity` によって差分種別がはっきり伝わります。
- **[EN]** Even when a hunk only edits prose, the quick differ emits `textual_changes` entries with `text_change_type` (added / removed / modified) plus `line_similarity`, keeping text-only shifts visible to downstream tooling.
- **[FR]** Les changements purement textuels produisent aussi des entrées `textual_changes` annotées par `text_change_type` (added / removed / modified) et `line_similarity`, ce qui maintient la visibilité côté aval.
