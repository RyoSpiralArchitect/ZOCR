...
- **[JA]** `toy_memory.json`（既定保存先。`ZOCR_TOY_MEMORY` で変更可）にグリフアトラスと N-gram を永続化し、Toy OCR が前回学習した文字や語彙を次回以降も参照します。サマリーの `toy_memory` には読込結果（`load`）と実行前後のスナップショット差分（`before_run` / `after_run` / `delta_run`）、保存結果（`save`）に加えて、今回の推論で観測した平均信頼度・言語的コヒーレンス・サプライズ値や高サプライズ例 (`recognition`) が記録され、Toy の学習量と“違和感”の両方を可視化できます。
- **[EN]** The toy OCR persists its glyph atlas and N-gram priors to `toy_memory.json` (override via `ZOCR_TOY_MEMORY`), so previously seen glyphs/vocabulary are reused. The `toy_memory` block in the summary now captures the load outcome, before/after snapshots with per-run deltas, the save status, and a `recognition` section that tracks average confidence/coherence/surprisal plus notable outliers from the current run, helping you gauge both learning and lingering uncertainty.
- **[FR]** Le toy OCR conserve son atlas de glyphes et ses N-grammes dans `toy_memory.json` (chemin modifiable via `ZOCR_TOY_MEMORY`), réutilisant les caractères observés. La section `toy_memory` du résumé consigne désormais le résultat du chargement, les instantanés avant/après avec les deltas par exécution, l’état de sauvegarde et un bloc `recognition` qui mesure la confiance/cohérence/surprise moyennes ainsi que les exemples atypiques du run courant, afin de suivre l’apprentissage tout en détectant les doutes persistants.
