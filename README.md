...
- **[JA]** `--out_agentic`（および orchestrator の `run_diff`）は AgenticRAG 連携用の `agentic_requests.json` を生成し、各イベントに「JA/EN でどう説明するか」「どんな差分画像/GUI を描画するか」というプロンプトとハイライト指示をまとめます。
- **[EN]** Add `--out_agentic` (and note that `run_diff` writes the same file) to produce `agentic_requests.json`, a bundle of bilingual prompts plus visual-overlay briefs so Agentic RAG/GUI workers can render diff images or spoken explanations straight from the events.
- **[FR]** L’option `--out_agentic` (et `run_diff`) génère `agentic_requests.json`, c’est‑à‑dire des requêtes prêtes pour l’Agentic RAG avec instructions bilingues et consignes de rendu visuel/diff afin de produire images ou narratifs sans rouvrir le document.
- **[JA]** `--out_markdown` を指定するか orchestrator の `run_diff` を使うと、人間レビューや Slack/Teams 転送向けの `report.md` が生成されます（同じ Markdown テキストは handoff bundle 内の `markdown_report` にも埋め込まれます）。
- **[EN]** Add `--out_markdown` or rely on the orchestrator’s `run_diff` to obtain `report.md`, a ready-to-share Markdown digest for chats/dashboards (the same text is embedded as `markdown_report` inside the handoff bundle).
- **[FR]** En ajoutant `--out_markdown` (ou via `run_diff`), on produit `report.md`, un résumé Markdown prêt pour Slack/Teams ; ce texte est également inclus dans `handoff_bundle.json` via le champ `markdown_report`.
- **[JA]** `--out_bundle` を付けるとイベント / unified diff 文字列 / assist plan / agentic requests を 1 つにまとめた `handoff_bundle.json` が得られ、API や GUI から一括取り込みできます（`run_diff` も同ファイルを出力）。
- **[EN]** Add `--out_bundle` to persist `handoff_bundle.json`, a single payload combining the events, unified diff text, assist plan, agentic requests, and the Markdown digest so APIs/GUI front-ends can ingest everything at once (the orchestrator’s `run_diff` emits the same artifact).
- **[FR]** L’option `--out_bundle` génère `handoff_bundle.json`, un paquet unique regroupant événements, diff texte, plan d’assistance, requêtes agentic et le résumé Markdown afin que l’API/la GUI chargent tout en une fois (le `run_diff` de l’orchestrateur produit le même fichier).
- **[JA]** 差分対象ドキュメントの許容枚数上限を撤廃し、`DiffAssistPlanner` のキューも無制限にしたため、10数件の実行結果をまとめて比較しても切り捨てが発生しません。軽量 differ も RapidFuzz ベースの行類似キャッシュを使うので、多ページの ToyOCR テキストでも高速のままです。
- **[EN]** The per-bucket cap inside `DiffAssistPlanner` has been removed, so you can diff dozens of documents in a single sweep without losing events. The quick differ now caches RapidFuzz-backed line signatures, keeping multi-page ToyOCR/text comparisons fast even as the input count grows.
- **[FR]** Les limites par compartiment du `DiffAssistPlanner` ont disparu : on peut comparer des dizaines de documents en une passe sans échantillonnage. Le diff léger exploite désormais RapidFuzz et met en cache les signatures de lignes, ce qui maintient la vitesse même lorsque plusieurs pages ou exports ToyOCR sont analysés.
- **[JA]** `summary.numeric_summary` には通貨/単位ごとのバケットと最大の金額差トップ5が入り、Slack などで「今月は USD +12,000 / EUR -8,500」と即把握できます（簡易 diff でも同じ構造が入ります）。
- **[EN]** The `summary.numeric_summary` payload now captures net/absolute totals plus the top currency/unit buckets and the five largest swings, so dashboards can instantly state “USD +12,000 / EUR -8,500”; the quick differ feeds the same structure.
- **[FR]** Le champ `summary.numeric_summary` récapitule désormais les totaux nets/absolus, les compartiments par devise/unité et les cinq variations majeures pour annoncer instantanément « USD +12 000 / EUR −8 500 » — le diff léger produit la même structure.
- **[JA]** `summary.textual_summary` には `text_change_type` 別件数・平均類似度/overlap/Jaccard・頻出トークン（追加/削除/共通）・文章差し替えトップ例が含まれ、文章メモの追記や条文改定も dashboard / handoff bundle のみで把握できます。
- **[EN]** `summary.textual_summary` lists counts per `text_change_type`, average similarity/overlap/jaccard, token highlights (added/removed/common), and the most notable rewrites so even a dashboard or handoff bundle immediately surfaces memo updates or clause edits.
- **[FR]** `summary.textual_summary` regroupe les volumes par `text_change_type`, les similarités/overlaps/jaccards moyens, les tokens marquants (ajoutés/supprimés/communs) et les principales réécritures afin que les mémos ou clauses modifiés ressortent directement dans les tableaux de bord ou bundles.
- **[JA]** `summary.section_summary` で章・節ごとのイベント件数、数値/テキスト内訳、代表行、累積差額を一覧化でき、`report.md` や handoff bundle だけで「第3章が+¥1.2M / 文章差分4件」と即把握できます。
- **[EN]** `summary.section_summary` captures per-section counts, numeric/textual splits, representative rows, and cumulative deltas so `report.md` or the handoff bundle alone can report “Chapter 3: +¥1.2M / 4 textual edits” without reopening the source files.
- **[FR]** `summary.section_summary` indique pour chaque section le volume d'événements, la répartition numérique/texte, quelques exemples et le delta cumulé, de sorte que `report.md` ou le handoff bundle suffisent pour lire « Chapitre 3 : +1,2 M¥ / 4 révisions textuelles ».
- **[JA]** `assist_plan.json` には `impact_summary`（総インパクトスコア・バケット別件数・トップエントリ）が追加され、各 recommendation / agentic request にも `impact_score` / `impact_bucket` が付与されるため、GUI や API で重要度順の並べ替えが容易になりました。
- **[EN]** `assist_plan.json` now ships an `impact_summary` (total impact score, bucket counts, top entries) and every recommendation/agentic request exposes `impact_score` + `impact_bucket`, letting GUIs/APIs rank the diff feed by urgency.
- **[FR]** `assist_plan.json` inclut désormais `impact_summary` (score total, comptes par compartiment, entrées majeures) et chaque recommandation/requête agentic fournit `impact_score` / `impact_bucket`, ce qui simplifie le tri des diffs par importance côté GUI/API.
...
- **[JA]** 軽量 differ でも `--simple_agentic_out` を指定すれば `agentic_requests.json` を個別に出力でき、ToyOCR 由来のイベントをそのまま AgenticRAG の差分画像生成・口頭説明タスクに渡せます。
- **[EN]** Supply `--simple_agentic_out` when running only the quick differ to export its `agentic_requests.json`, giving ToyOCR-style events the same visual/explanation prompts for downstream Agentic RAG flows.
- **[FR]** Avec `--simple_agentic_out`, le diff léger produit lui aussi `agentic_requests.json`, ce qui permet d’alimenter directement l’Agentic RAG (images de diff, explications orales) à partir des sorties ToyOCR.
- **[JA]** `--simple_bundle_out` を使えば軽量 differ 版の `handoff_bundle.json` も保存でき、ToyOCR で拾った差分を API / GUI / AgenticRAG へワンショットで渡せます。
- **[EN]** Use `--simple_bundle_out` to capture a quick-differ `handoff_bundle.json`, letting ToyOCR-style changes (plus the Markdown digest) flow into APIs, GUI cards, or Agentic RAG helpers from a single artifact.
- **[FR]** `--simple_bundle_out` enregistre la version légère de `handoff_bundle.json`, de sorte que les diffs issus de ToyOCR (ainsi que le résumé Markdown) soient livrés en une seule fois aux API/GUI ou aux agents RAG.
- **[JA]** `--simple_markdown_out` で軽量 differ 専用の `report.md` も書き出され、git 風 diff を開かなくても差分の要約を Slack や顧客報告に貼り付けられます。
- **[EN]** Supply `--simple_markdown_out` to emit a quick-differ `report.md`, giving chats or status threads a git-style summary without reopening the raw diff.
- **[FR]** Avec `--simple_markdown_out`, le diff léger exporte aussi `report.md`, un résumé prêt à être partagé sans rouvrir le diff brut.
- **[JA]** 数値差分は置換行だけでなく、行追加/削除（例：新しい費目の請求、数量行の抜け落ち）でも検出されるため、git diff と同じ見た目で実額の増減が一目で分かります。
- **[EN]** Numeric deltas are now captured for replacements *and* inserted/deleted lines (e.g., a brand-new fee line or a missing quantity), so the git-style view always reports the real amount changes.
- **[FR]** Les deltas numériques couvrent désormais les lignes insérées/supprimées (nouveau poste de facturation, quantité disparue) en plus des remplacements, ce qui rend l’augmentation réelle immédiatement visible dans le diff façon git.
- **[JA]** 行マッチングは数値を `<num>` として正規化したテキスト類似度で行われ、スペース差や列順入れ替えがあっても同じ行として結びつけます。結果 JSON には `line_signature` / `line_label` / `line_similarity` も含まれ、RAG 側で即参照できます。
- **[EN]** Line pairing now ignores numeric tokens by normalizing them to `<num>` before running fuzzy matching, so cosmetic spacing tweaks or mild reordering still map to the same row. The quick-diff JSON ships `line_signature`, `line_label`, and `line_similarity` fields so RAG consumers can anchor the change instantly.
- **[FR]** L’alignement des lignes remplace les nombres par `<num>` avant de mesurer la similarité, ce qui maintient l’appariement même si l’espacement change ou si l’ordre des lignes bouge. Le JSON du diff léger expose aussi `line_signature`, `line_label` et `line_similarity` pour que les agents RAG identifient immédiatement chaque modification.
- **[JA]** 数値が登場しない注記や文章差し替えでも `textual_changes` エントリが生成され、`text_change_type`（added / removed / modified）と `line_similarity` で「行の追加か文章変更か」を判別しつつ、`text_token_stats`（追加/削除/共通トークン + overlap/jaccard）と `text_highlight`（`[[…]]` で囲ったミニ差分）も併せて下流へ渡します。
- **[EN]** Even for prose-only hunks, the quick differ emits `textual_changes` entries with `text_change_type`, `line_similarity`, and now `text_token_stats` (added/removed/common tokens plus overlap/jaccard) alongside `text_highlight` snippets that bracket edits with `[[…]]`, giving downstream LLM/RAG helpers a compact view of what changed.
- **[FR]** Même sans chiffres, les entrées `textual_changes` incluent `text_change_type`, `line_similarity`, mais aussi `text_token_stats` (tokens ajoutés/supprimés/communs + overlap/jaccard) et `text_highlight` (aperçu `[[…]]`), de sorte que les agents LLM/RAG perçoivent immédiatement la nature de la réécriture.
- **[JA]** 通貨記号（$ / € / ¥ など）や通貨コード（USD / EUR / 円）、`千` / `万` / `億` / `k` / `M` / `B`、括弧付きマイナス、% / ％ を含む行でも正しく金額・比率を抽出し、イベントに `numeric_unit` / `numeric_currency` / `numeric_is_percent` / `numeric_scale` を載せて下流へ伝えられます。
- **[EN]** Currency symbols/codes, Japanese `千` / `万` / `億`, western `k` / `M` / `B`, parenthetical negatives, and %/％ tokens are parsed automatically; every quick-diff event includes `numeric_unit`, `numeric_currency`, `numeric_is_percent`, and `numeric_scale` so downstream tooling keeps the unit context.
- **[FR]** Les symboles/codes de devise, les suffixes `千` / `万` / `億` ainsi que `k` / `M` / `B`, les montants négatifs entre parenthèses et les pourcentages %/％ sont désormais reconnus ; chaque événement du diff léger expose `numeric_unit`, `numeric_currency`, `numeric_is_percent` et `numeric_scale` pour préserver le contexte d’unité.
- **[JA]** `a_row_context` / `b_row_context` / `row_context_radius` で変更行の前後抜粋も保持し、差分周辺の原文を下流 LLM/RAG が即参照できます。
- **[EN]** Each event also carries `a_row_context`, `b_row_context`, and `row_context_radius`, exposing trimmed neighbor lines so downstream LLM/RAG helpers can inspect the surrounding text without reopening the files.
- **[FR]** Les événements transportent en plus `a_row_context`, `b_row_context` et `row_context_radius`, c’est-à-dire un extrait des lignes voisines pour que les agents LLM/RAG lisent le contexte local sans rouvrir les documents.
- **[JA]** Markdown の `#` / `##` や `===` 下線、`第3条` / `Section 2` などの見出しを自動検出し、イベント/assist plan に `section_heading` / `section_path` / `section_level`（および `_a` / `_b` 版）を付与するため、ToyOCR 由来のメモ比較でも「どの節で差分が起きたか」を即把握できます。
- **[EN]** The quick differ now scans Markdown hashes, Setext underlines, and legal-style headings such as `第3条` or `Section 2` so every event (and assist-plan entry) inherits `section_heading`, `section_path`, and `section_level` metadata—plus per-side `_a` / `_b` values—making it obvious which chapter the change belongs to even in ToyOCR exports.
- **[FR]** Le diff léger détecte désormais les titres Markdown (`#`, `##`, variantes Setext) ainsi que les en-têtes juridiques (`第3条`, `Section 2`…), puis ajoute `section_heading`, `section_path`, `section_level` (ainsi que les versions `_a` / `_b`) aux événements et plans d’assistance pour localiser immédiatement la section concernée dans les exports ToyOCR.
- **[JA]** 1 行に複数の数値がある場合は Hungarian 法ベースで最も近い組み合わせを選び、通貨/単位/パーセントの不一致にはペナルティを課します。許容コストは `--simple_pair_threshold` で調整でき、イベントには `line_pair_cost` / `line_pair_gap` / `line_pair_penalty` / `line_pair_status` が入り、どの値が対応付いたのかを即判断できます。
- **[EN]** When a line lists several numbers, the quick differ now runs a Hungarian-style assignment to pair the closest values and penalises unit/currency/percent mismatches. Tune the tolerance via `--simple_pair_threshold`; the emitted events surface `line_pair_cost`, `line_pair_gap`, `line_pair_penalty`, and `line_pair_status` so downstream tooling knows which numbers aligned and which remained unmatched.
- **[FR]** Dès qu’une ligne comporte plusieurs montants, le diff léger applique un appariement de type hongrois pour associer les valeurs les plus proches et ajoute des pénalités en cas d’unité/devise/pourcentage divergents. Ajustez la tolérance avec `--simple_pair_threshold`; les événements exposent `line_pair_cost`, `line_pair_gap`, `line_pair_penalty` et `line_pair_status`, ce qui indique quelles valeurs ont été appariées ou laissées orphelines.
- **[JA]** simple 用入力にはテキストファイルだけでなく `out/<run>/` のような実行ディレクトリも渡せ、`rag/bundle.md` → `bundle.md` → `preview.md` → `.txt` の順で自動解決します（ToyOCR で生成したままのフォルダを指すだけで比較可能）。
- **[EN]** The quick-differ inputs accept either explicit text files or a run directory such as `out/<run>/`; the CLI searches `rag/bundle.md`, `bundle.md`, `preview.md`, and `.txt` siblings in that order so ToyOCR folders can be diffed without extra prep.
- **[FR]** Les entrées du diff léger peuvent pointer vers un fichier texte ou directement vers un dossier d’exécution (`out/<run>/`) : la CLI y cherche successivement `rag/bundle.md`, `bundle.md`, `preview.md` ou les variantes `.txt`, ce qui permet de comparer un export ToyOCR tel quel.
...
- **[JA]** `assist_plan.json` には `domain_tags` / `llm_directive` / `domain_briefings` / `handoff_packets` がまとまり、請求書・契約・物流に加えて医療・保険・製造・エネルギー・コンプラ・不動産・通信・小売・官公庁・教育・テック・マーケ・航空・建設だけでなく、自動車・ホスピタリティ・メディア・銀行・ゲーム・飲食・農業・鉱業・海運・スポーツ・エンタメ・非営利・サイバーセキュリティ領域まで diff 専用テンプレでカバーします。
- **[EN]** Each assist plan entry still ships `domain_tags`, `llm_directive`, `domain_briefings`, and `handoff_packets`, and the heuristics now span invoice/contract/logistics plus healthcare/insurance/manufacturing/energy/compliance, real-estate/telecom/retail/public-sector/education/technology/marketing/aviation/construction, *and* the new automotive/hospitality/media/banking/gaming/food-beverage/agriculture/mining/shipping/sports/entertainment/nonprofit/cybersecurity verticals.
- **[FR]** Chaque plan d’assistance conserve `domain_tags`, `llm_directive`, `domain_briefings` et `handoff_packets`, tout en étendant la couverture : factures/contrats/logistique, santé/assurance/fabrication/énergie/conformité, immobilier/télécom/retail/secteur public/éducation/technologie/marketing/aviation/construction, et désormais automobile/hôtellerie/médias/banque/jeu vidéo/restauration-agro/agriculture/mines/shipping/sports/entertainement/organisations à but non lucratif/cybersécurité.
- **[JA]** `assist_plan.json` には AgenticRAG 連携用の `agentic_requests` も含まれ、`visual_brief` / `narrative_brief` / `handoff_focus` / `preferred_outputs` でどの行をハイライトし、どんな差分画像や音声説明を返すかを LLM に直接指示できます。
- **[EN]** The plan now exposes `agentic_requests` with `visual_brief`, `narrative_brief`, `handoff_focus`, and `preferred_outputs`, giving downstream Agentic RAG stacks ready-made prompts for diff overlays plus bilingual explanations without re-deriving context.
- **[FR]** `assist_plan.json` inclut désormais `agentic_requests` : chaque entrée fournit `visual_brief`, `narrative_brief`, `handoff_focus` et `preferred_outputs`, de sorte que les agents RAG/GUI génèrent images et narratifs directement depuis ces consignes.
- **[JA]** さらに各エントリに `a_row_context` / `b_row_context` / `row_context_radius` と `line_signature` / `text_highlight` / `text_token_stats` などの軽量 diff メタデータをコピーするため、RAG や補助チームは原文を開かずに周辺文脈とハイライトを確認できます。
- **[EN]** Entries now copy the lightweight diff metadata—`a_row_context`, `b_row_context`, `row_context_radius`, plus `line_signature`, `text_highlight`, `text_token_stats`, and friends—so RAG/reanalysis teams can inspect the surrounding text and inline highlights without reopening the source files.
- **[FR]** Chaque entrée réplique désormais les métadonnées du diff léger (`a_row_context`, `b_row_context`, `row_context_radius`, ainsi que `line_signature`, `text_highlight`, `text_token_stats`, etc.), ce qui permet aux équipes RAG/réanalyse de visualiser le contexte et les surlignages sans rouvrir les documents.
- **[JA]** すべての `cell_updated` イベントには 0〜1 の `confidence` スコアが付き、テキスト類似度・相対差分・金額規模をブレンドした信頼度を `report.md` / HTML / assist plan / agentic request へ引き継ぎます。
- **[EN]** Every `cell_updated` event now carries a 0–1 `confidence` score that blends text similarity gaps with relative/numeric deltas, and the value is forwarded to `report.md`, HTML cards, the assist plan, and the agentic requests.
- **[FR]** Chaque événement `cell_updated` expose désormais un score `confidence` (0 à 1) combinant écart de similarité et amplitude des deltas relatifs/numériques ; ce signal est repris dans `report.md`, le rapport HTML, le plan d’assistance et les requêtes agentic.
- **[JA]** `summary.impact_score_total` / `impact_bucket_counts` や `impact_summary.top_entries` に加え、各 recommendation・agentic request に `impact_score` / `impact_bucket` が付くため、ダッシュボードや API で重要度順に並べ替えて即時エスカレーションできます。
- **[EN]** With `summary.impact_score_total`, `impact_bucket_counts`, `impact_summary.top_entries`, and per-entry `impact_score` / `impact_bucket`, dashboards and APIs can sort the diff feed by urgency and escalate the most critical deltas instantly.
- **[FR]** Grâce à `summary.impact_score_total`, `impact_bucket_counts`, `impact_summary.top_entries` et aux champs `impact_score` / `impact_bucket` par entrée/requête agentic, les tableaux de bord/API classent désormais les diffs selon leur criticité pour accélérer les escalades.
