...
- **[JA]** 再解析 (`reanalyze_learning_jsonl`) は Tesseract があれば追加エンジンとして併用し、未導入でも自前の合成フォールバック（オートコントラスト＋拡大・アンシャープなど）で同等の候補生成を行います。文字のゆらぎ辞書で `??I` → `771` などの揺れも補正し、利用した外部（またはフォールバック）エンジン数をサマリーに記録します。
- **[EN]** The reanalysis stage (`reanalyze_learning_jsonl`) taps Tesseract when available, and if it is missing the pipeline falls back to synthetic heuristics (autocontrast, padding, scaling, unsharp masking) to keep multi-engine style proposals coming. The ambiguity map still turns glyph noise like `??I` into `771`, and the summary records which external or fallback engines contributed fixes.
- **[FR]** La phase de réanalyse (`reanalyze_learning_jsonl`) exploite Tesseract lorsqu’il est présent et, en son absence, bascule vers un moteur de secours synthétique (auto-contraste, agrandissement, renforcement du contraste) pour proposer des variantes comparables. La carte d’ambiguïtés convertit toujours des bruits tels que `??I` en `771`, et le résumé indique les moteurs externes ou de secours utilisés.
