...
- **[EN]** Add `--out_bundle` to persist `handoff_bundle.json`, a single payload combining the events, unified diff text, assist plan, and agentic requests so APIs/GUI front-ends can ingest everything at once (the orchestrator’s `run_diff` emits the same artifact).
- **[FR]** L’option `--out_bundle` génère `handoff_bundle.json`, un paquet unique regroupant événements, diff texte, plan d’assistance et requêtes agentic afin que l’API/la GUI chargent tout en une fois (le `run_diff` de l’orchestrateur produit le même fichier).
...
- **[EN]** Use `--simple_bundle_out` to capture a quick-differ `handoff_bundle.json`, letting ToyOCR-style changes flow into APIs, GUI cards, or Agentic RAG helpers from a single artifact.
- **[FR]** `--simple_bundle_out` enregistre la version légère de `handoff_bundle.json`, de sorte que les diffs issus de ToyOCR soient livrés en une seule fois aux API/GUI ou aux agents RAG.
