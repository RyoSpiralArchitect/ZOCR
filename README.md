...
- **[JA]** Export 後は `doc.contextual.jsonl.quality.json` が自動生成され、低信頼率・review 率・高 surprisal 率・ページ画像欠落・guard timeout・blank 比率を `pass/warn/fail` とスコアで判定します。`*.signals.json` と `last_export_stats()` にも `quality_status` / `quality_score` / レポートパスが残るため、後段ジョブの品質ゲートとして使えます。<br>**[EN]** Each export now writes `doc.contextual.jsonl.quality.json`, a machine-readable gate report that scores low confidence, review ratio, high surprisal, missing bitmaps, guard timeouts, and blank-crop rate as `pass`/`warn`/`fail`; `*.signals.json` and `last_export_stats()` expose the same status, score, and report path for downstream job gates.
