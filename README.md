...
- **[JA]** 数値が登場しない注記や文章差し替えでも `textual_changes` エントリが生成され、`text_change_type`（added / removed / modified）と `line_similarity` で「行の追加か文章変更か」を下流 LLM/RAG が判別できます。
- **[EN]** Even when a hunk only tweaks prose (no numbers), the quick differ now emits `textual_changes` entries with `text_change_type` (added / removed / modified) plus `line_similarity`, so downstream LLM/RAG flows still see a structured signal.
- **[FR]** Même sans nombres, les différences purement textuelles produisent des entrées `textual_changes` annotées par `text_change_type` (added / removed / modified) et `line_similarity`, ce qui donne un signal structuré aux agents LLM/RAG.
